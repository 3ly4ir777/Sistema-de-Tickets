...
export default function ReportesPage() {
  const exportCsv = async () => {
    try {
      ...
      const { data, error } = await supabase.from('tickets').select('id,code,title,priority,status,created_at,updated_at');
      if (error) throw error;
      const rows = data || [];
      const header = ['id','code','title','priority','status','created_at','updated_at'];
      const csv = [header.join(',')].concat(rows.map((r: any) => header.map(h => `"${(r[h] ?? '').toString().replace(/"/g,'""')}"`).join(','))).join('\n');
          ...
          <main className="p-6 max-w-[1600px] w-full mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500">Exporta auditorías en formato CSV sobre la velocidad de la mesa de soporte.</p>
              </div>
              <button onClick={exportCsv} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg shadow-sm transition-colors">
                <Download size={16} /> Exportar Datos (CSV)
              </button>
            </div>
